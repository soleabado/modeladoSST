...
EmberHalo3D26Generator::EmberHalo3D26Generator(SST::ComponentId_t id, Params& params) :
	...
	m_loopIndex(0)
{
	nx  = (uint32_t) params.find("arg.nx", 128);
	ny  = (uint32_t) params.find("arg.ny", 128);
	nz  = (uint32_t) params.find("arg.nz", 128);

	peX = (uint32_t) params.find("arg.pex", 16);
	peY = (uint32_t) params.find("arg.pey", 32);
	peZ = (uint32_t) params.find("arg.pez", 32);

	items_per_cell = (uint32_t) params.find("arg.fields_per_cell", 10);
