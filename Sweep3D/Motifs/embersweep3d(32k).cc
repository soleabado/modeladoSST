...
EmberSweep3DGenerator::EmberSweep3DGenerator(SST::ComponentId_t id, Params& params) :
    ...
    m_InnerLoopIndex(0)
{
	px = (int32_t) params.find<int32_t>("arg.pex", 128);
	py = (int32_t) params.find<int32_t>("arg.pey", 256);

	iterations = (uint32_t) params.find<uint32_t>("arg.iterations", 1);

	nx  = (uint32_t) params.find<uint32_t>("arg.nx", 40);
	ny  = (uint32_t) params.find<uint32_t>("arg.ny", 40);
	nz  = (uint32_t) params.find<uint32_t>("arg.nz", 1000);
	kba = (uint32_t) params.find<uint32_t>("arg.kba", 10);

	data_width = (uint32_t) params.find<uint32_t>("arg.datatype_width", 8);
	fields_per_cell = (uint32_t) params.find<uint32_t>("arg.fields_per_cell", 1);

	double flops_per_cell = params.find<double>("arg.flops_per_cell", 26.0);
